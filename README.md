...
9. [비밀 기능 및 차별화 포인트](#9-비밀-기능-및-차별화-포인트)
10. [발표 시연 순서](#10-발표-시연-순서)
11. [자주 발생한 오류와 해결 방법](#11-자주-발생한-오류와-해결-방법)
12. [주의사항](#12-주의사항)
...
6. 필요하면 삭제, 복구, 마스킹, 캐니에지, 필터링을 적용합니다.
...
> 단순히 이미지를 보여주는 UI가 아니라, 학습 데이터 품질을 높이는 데이터 엔지니어링 기능을 구현했습니다.
...
> 학습 전 데이터를 직접 전처리할 수 있어 모델이 불필요한 특징을 학습하는 것을 줄일 수 있습니다.
...
> 도로선과 가장자리 정보를 강조해 주행 방향 판단에 도움이 되는 특징을 시각적으로 확인할 수 있습니다.
...
> 숫자로만 보이던 angle/throttle을 이미지 위 방향 정보로 바꿔 사용자가 직관적으로 이해할 수 있게 했습니다.
...
> 학습 실행만 하고 끝나는 것이 아니라, 학습 결과를 다시 UI로 불러와 분석하고 확인할 수 있게 했습니다.
...
> 특정 PC에만 고정된 도구가 아니라, 팀원마다 다른 사용자명과 경로를 입력해 사용할 수 있도록 일반화했습니다.

---

## 10. 발표 시연 순서

아래 순서대로 시연하면 프로젝트의 기능 흐름이 자연스럽게 보입니다.

### 10.1 1분 요약 시연

1. `폴더 열기`로 DonkeyCar 데이터 불러오기
2. 프레임 목록과 이미지 미리보기 확인
3. `이상 탐지 실행`으로 이상 후보 표시
4. 이미지 일부를 드래그해 `영역 가리기` 실행
5. 삭제된 프레임과 편집된 프레임 색상 확인
6. `AI 학습` 창에서 명령 생성과 성공률 화면 설명

### 10.2 3분 기능 시연

1. 데이터 로드 후 angle/throttle 그래프 설명
2. 자동 재생과 재생 속도 조절 설명
3. 이상 탐지 결과를 `다음 이상`으로 이동하며 확인
4. 불량 프레임을 `이미지 삭제`하고 빨간색 상태 확인
5. `삭제 이미지 복구`로 복구 확인
6. 중앙 이미지에서 마스킹/교체 설명
7. `전체사진 캐니 미리`와 `선택사진 캐니 변경` 차이 설명
8. `방향/스로틀` 오버레이로 주행 방향 설명
9. `AI 학습`에서 학습 범위와 실행 환경 설명
10. `_training_runs`와 학습 로그 분석 설명

### 10.3 5분 발표용 설명 흐름

1. DonkeyCar는 이미지와 angle/throttle 데이터를 학습한다.
2. 그래서 학습 데이터 품질이 모델 성능에 직접 영향을 준다.
3. 기존 명령어 기반 workflow는 초보자가 데이터를 정리하기 어렵다.
4. 우리 도구는 WinForms로 데이터를 시각화하고 정제한다.
5. 삭제/복구, 마스킹/교체, 캐니에지로 학습 전처리를 지원한다.
6. 이상 탐지로 사람이 일일이 찾기 어려운 불량 구간을 자동 추천한다.
7. AI 학습 창은 Windows/WSL/VirtualBox 경로를 자동 생성한다.
8. 특정 사용자명에 고정하지 않고 각자의 Ubuntu user와 경로를 입력할 수 있다.
9. 학습 중 로그를 저장하고, 중단돼도 progress.json과 로그를 남긴다.
10. 학습 후 로그 분석과 방향 프리뷰로 결과를 다시 검토할 수 있다.

---

## 11. 자주 발생한 오류와 해결 방법

### 11.1 `Permission denied (publickey,password,keyboard-interactive)`

원인:

- SSH 사용자명 또는 비밀번호 오류
- Ubuntu SSH 서버 설정에서 비밀번호 로그인이 비활성화됨
- VirtualBox 포트포워딩은 되지만 계정 인증 실패
...
### 11.2 `No config file at location: ./config.py`

원인:

`python train.py` 또는 `donkey train` 실행 위치가 `mycar` 폴더가 아닙니다.
...
Form2의 `원격 mycar`도 반드시 `config.py`와 `train.py`가 있는 폴더여야 합니다.

### 11.3 `python: can't open file 'train.py'`

원인:

학습 데이터셋 폴더(`data/_training_sets/...`) 안에서 `python train.py`를 실행하려고 했습니다.
...
```bash
cd /media/sf_virtualbox_share/mycar
python train.py --tub /media/sf_virtualbox_share/mycar/data/_training_sets/... --model /media/sf_virtualbox_share/mycar/models/mypilot.h5
```

### 11.4 `manifest.json` 없음

원인:

학습 데이터셋 생성 시 DonkeyCar v5 tub에 필요한 `manifest.json`이 누락되었습니다.

현재 버전은 `_training_sets` 생성 시 `manifest.json`과 `catalog_0.catalog_manifest`를 함께 생성하도록 수정되어 있습니다. 기존에 만들어진 오래된 `_training_sets` 폴더를 쓰지 말고 새로 `학습 데이터 생성`을 누르세요.

### 11.5 깨진 이미지 또는 누락 이미지

오류 예시:

```text
PIL.UnidentifiedImageError: cannot identify image file
FileNotFoundError: No such file or directory: data/images/10_cam_image_array_.jpg
```

해결:

- UI에서 해당 프레임을 삭제 처리
- `삭제 이미지 복구`로 복구 가능한지 확인
- 학습 데이터 생성 시 삭제된 프레임 제외
- 깨진 이미지를 직접 제거 또는 교체

### 11.6 TensorFlow `Illegal instruction`

원인:

VirtualBox VM에서 pip TensorFlow가 요구하는 AVX 명령어를 CPU가 제공하지 못하는 경우입니다.

해결 방향:

- pip TensorFlow 대신 conda-forge의 CPU TensorFlow 사용
- 검증된 환경 예시:

```bash
conda create -n donkey_conda_tf python=3.8 -y
conda activate donkey_conda_tf
conda install -c conda-forge tensorflow-cpu=2.10 keras=2.10 numpy=1.23.5 pillow h5py matplotlib pandas scikit-learn -y
pip install donkeycar==5.0.0
...
### 11.7 `No module named 'cv2'`

원인:

OpenCV Python 모듈이 없습니다.
...
### 11.8 `No module named 'albumentations'`
...
### 11.9 `localhost:8887` 접속 불가

원인:

- DonkeyCar drive 프로세스가 오류로 종료됨
- Windows 8887 포트가 이미 사용 중
- SSH 터널이 열리지 않음
...
### 11.10 웹 컨트롤 화면은 뜨는데 검은 화면만 보임

원인:

`CAMERA_TYPE MOCK` 상태이거나 시뮬레이터/카메라 입력이 없습니다.

확인:

```text
cfg.CAMERA_TYPE MOCK
Adding part MockCamera
```

시뮬레이터를 사용하려면 `DONKEY_GYM=True`와 `SIM_HOST`, `SIM_PORT` 설정을 확인해야 합니다.

---

## 12. 주의사항

### 12.1 삭제와 편집은 실제 파일에 영향을 줍니다
...
발표에서는 다음 한 문장으로 요약할 수 있습니다.

> DonkeyCar UI 데이터 관리 도구는 자율주행 모델 학습 전 데이터를 시각적으로 정제하고, 환경별 학습 명령을 자동화하며, 학습 결과를 다시 분석할 수 있게 만든 WinForms 기반 통합 학습 데이터 플랫폼입니다.
